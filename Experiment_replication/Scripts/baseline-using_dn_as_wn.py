...
def main(fileQuery,fileDataset):
	...
	if fileQuery == "c2":
		fileQuery = 'triangle_count_T.json'
		fq = 'C2'
	else:
		fileQuery = 'twitter_selfjoin-count_T.json'
		fq = 'C1'
	if fileDataset == "full":
	...
	nameFileResult = pwd+'Experiment_replication/Results/'+sample+'_'+str(time.strftime("%d%b%Y-%Hh%Mm%Ss"))+'_DS-'+ds+'_Q-'+fq+'.json'
		...
		for s in avgTime:
			...
			while errorDeploy:
				...
				if not errorDeploy:
					...
					filesIngest = ['ingest_twitter_T1','ingest_twitter_T2','ingest_twitter_T3']
					ingest_and_query(s['schema'],path,fileQuery,fileDataset,filesIngest)
