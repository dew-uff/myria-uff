				...
				if not errorDeploy:
					...
					if not queryERROR:
						...
						for outT in outQ:
							...
							timeQuery.append({'query':outT['query'],'start':st.strftime("%H:%M:%S.%f"), 'end':ft.strftime("%H:%M:%S.%f"),'time': (ft - st).total_seconds()})

					if (not queryERROR) and (not errorDeploy):
						s['steps'].append({'id':x,'timeQuery':timeQuery,'timeTotal':(max(datetime.datetime.strptime(t['end'],"%H:%M:%S.%f") for t in timeQuery) - min(datetime.datetime.strptime(t['start'],"%H:%M:%S.%f") for t in timeQuery)).total_seconds(),'finishTime':time.strftime("%Hh%Mm%Ss")})
						#Salva resultados no cenário no arquivo
						writeJson(nameFileResult,avgTime)

	#calcula média das consultas para cada esquema
	for s in avgTime:
		timeTotal = [t['timeTotal'] for t in s['steps']]
