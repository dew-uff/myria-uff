...
def main():
	...
	listDN = ['localhost','localhost','localhost','localhost']
	listMaq = ['localhost','localhost','localhost','localhost']
	hostname = 'localhost'
	path = '/home/frankwrs/myria/'
	...
	while n<=2:
		...
		#prepara o deploy
		prepareDeploy(path, hostname,listDeploy,8001)
		...
		for s in schemas:
			...
			#for node in listDN:
				#subp.call('ssh '+node+' \'rm -Rf /tmp/myria/*\'', shell=True)
