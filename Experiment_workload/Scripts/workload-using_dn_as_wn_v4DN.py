...
def prepareDeploy(path, master,listDeploy,port,s):
	...
	file = open(path+'myriadeploy/deployment4DN.cfg', 'r')
	...
	file = open(path+'myriadeploy/deployment4DN.cfg', 'w')
...
def deployMyria(stopAll, cleanAll, setupCluster, startDeploy, pwd, master, listDN, s):
	...
	setup_cluster = ['./setup_cluster.py deployment4DN.cfg']
	deploy = ['./launch_cluster.sh deployment4DN.cfg']
	...
	if stopAll:
		...
		cmd = ['./stop_all_by_force.py deployment4DN.cfg;']
		#cmd = ['killall java']
			...
			for query in config["queries"]:
				...
				while errorDeploy:
					...
					if not errorDeploy:
						...
						os.chdir(path+"jsonQueries/tpcds/ingest4DN")
