...
def alter_join(dn,wn,path,filesQuery):
	for file in filesQuery:
		path_ = path+'jsonQueries/tpcds/query/'+file+'.json'
...
def alter_ingest(dn,path,fileDataset,filesIngest):
	for file in filesIngest:
		...
		data['workers'] = dn
		data['source']['filename'] = path+'jsonQueries/tpcds/ingest/data/small-'+fileDataset+'.dat_'
	writeJson(path_,data)

#funcao que alterar os json de ingest e query de acordo
#com o cenario
def ingest_and_query(schema,path,filesQuery,filesDataset,filesIngest):
	#schema = ns, rf, wn
	#ingest
	dn = list(range(1,schema['dn']+1))
	alter_ingest(dn,path,filesDataset,filesIngest)
	#join
	if (schema['wn']==0):
		wn = dn
	else:
...
def main(fileconf):
	...
	schemas = [{'m':2,'dn':2,'wn':0}]

	#Define nome do arquivo com resultados
	nameFileResult = pwd+'Experiment_workload/Results/'+sample+'_'+str(time.strftime("%d%b%Y-%Hh%Mm%Ss"))+'_DS-'+ds+'.json'
		...
		for s in avgTime:
			...
			while errorDeploy:
				...
				if not errorDeploy:
					...
					os.chdir(path+"jsonQueries/tpcds/")
					...
					for f in filesIngest:
						...
						#for f in filesIngest:
						#pipe Ingest
						ingest = ['curl -i -XPOST '+master+':8753/dataset -H \"Content-type: application/json\" -d @./'+f+'.json']
